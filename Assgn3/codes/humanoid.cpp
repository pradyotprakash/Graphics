#include "humanoid.hpp"
	...
	csX75::HNode* Humanoid::create_elem(std::vector <glm::vec4> v_positions, std::vector <glm::vec4> v_colors, std::vector<glm :: vec4> v_normals, int elem){
		...
		if(elem == HIP){
			...
			parent = l_leg1;
			foot1 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = foot1;
		}
		else if(elem == FOOT_RIGHT){
			parent = l_leg2;
			foot2 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = foot2;
		}
		else if(elem == SHOULDER){
			parent = torso;
			shoulder = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = shoulder;
		}
		else if(elem == NECK){
			parent = shoulder;
			neck = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = neck;
		}
		else if(elem == UPPER_ARM_LEFT){
			parent = shoulder;
			up_arm1 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = up_arm1;
		}
		else if(elem == UPPER_ARM_RIGHT){
			parent = shoulder;
			up_arm2 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = up_arm2;
		}
		else if(elem == ELBOW_LEFT){
			parent = up_arm1;
			elbow1 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = elbow1;
		}
		else if(elem == ELBOW_RIGHT){
			parent = up_arm2;
			elbow2 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = elbow2;
		}
		else if(elem == LOWER_ARM_LEFT){
			parent = elbow1;
			l_arm1 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = l_arm1;
		}
		else if(elem == LOWER_ARM_RIGHT){
			parent = elbow2;
			l_arm2 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = l_arm2;
		}
		else if(elem == HEAD){
			parent = neck;
			...
			parent = l_arm1;
			wrist1 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = wrist1;
		}
		else if(elem == WRIST_RIGHT){
			parent = l_arm2;
			wrist2 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = wrist2;
		}
		else if(elem == LIGHT_SABER_LEFT){
			parent = wrist1;
			saber1 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = saber1;
		}
		else if(elem == LIGHT_SABER_RIGHT){
			parent = wrist2;
			saber2 = new csX75::HNode(elem, parent, v_positions.size(), &v_positions[0], &v_colors[0], &v_normals[0], sizeOfvec4*v_positions.size(), sizeOfvec4*v_colors.size(), sizeOfvec4*v_normals.size());
			curr = saber2;
		}
		return curr;
	}
